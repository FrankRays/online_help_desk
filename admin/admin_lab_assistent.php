...
$user=$_POST['username'];
$pas=$_POST['password'];
$sql1=mysql_query("SELECT * FROM lab_assistent WHERE username='$user'");
 $result=mysql_fetch_array($sql1);
 if($result>0){
$message="<font color=blue>sorry the username entered already exists</font>";
 }else{
$sql=mysql_query("INSERT INTO lab_assistent(first_name,last_name,department,address,phone,email,username,password,date)
VALUES('$fname','$lname','$sid','$postal','$phone','$email','$user','$pas',NOW())");
if($sql>0) {
	$mess1= "OHD@ MAIT Account Info TYPE-LAB ASSISTENT";
	...
	header("location:http://".$_SERVER['HTTP_HOST'].dirname($_SERVER['PHP_SELF'])."/admin_lab_assistent.php");
}else{
...
<hr/>	
    <div class="tabbed_area">  
		...
		*/
        ...
        $result = mysql_query("SELECT * FROM lab_assistent") 
                or die(mysql_error());
        ...
        while($row = mysql_fetch_array( $result )) {
                ...
                echo '<td>' . $row['lab_ass_id'] . '</td>';
                echo '<td>' . $row['first_name'] . '</td>';
				echo '<td>' . $row['last_name'] . '</td>';
				echo '<td>' . $row['username'] . '</td>';
				?>
				<td><a href="update_lab_ass.php?username=<?php echo $row['username']?>"><img src="../images/update-icon.png" width="35" height="35" border="0" /></a></td>
				<td><a href="delete_lab_ass.php?lab_ass_id=<?php echo $row['lab_ass_id']?>"><img src="../images/delete-icon.jpg" width="35" height="35" border="0" /></a></td>
				<?php
		 } 
        // close table>
        echo "</table>";
?> 
		...
		echo $message;
			if(!empty($message1))
			  echo $message1;
			  ?>
		<form name="form1" onsubmit="return validateForm(this);" action="admin_lab_assistent.php" method="post" >
